...
bool CreateRenderingAttachments(
    RenderContext* pRenderContext, Image& colorAttachment, Image& depthAttachment)
{
    auto CreateAttachment = [&](Image& attachment, VkFormat imageFormat,
                                VkImageUsageFlags imageUsageFlags, VkImageAspectFlags imageAspect) {
        ...
        Check(vmaCreateImage(pRenderContext->GetAllocator(), &imageInfo, &imageAllocInfo,
                  &attachment.image, &attachment.imageAllocation, VK_NULL_HANDLE),
            "Failed to create attachment allocation.");

#ifdef _DEBUG
        auto attachmentName = std::format(
            "{} Attachment", imageAspect & VK_IMAGE_ASPECT_COLOR_BIT ? "Color" : "Depth");
        NameVulkanObject(pRenderContext->GetDevice(), VK_OBJECT_TYPE_IMAGE,
            (uint64_t)attachment.image, attachmentName);
#endif
        ...
        Check(vkCreateImageView(
                  pRenderContext->GetDevice(), &imageViewInfo, nullptr, &attachment.imageView),
            "Failed to create attachment view.");
    };

    CreateAttachment(colorAttachment, VK_FORMAT_R8G8B8A8_UNORM,
        VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
        VK_IMAGE_ASPECT_COLOR_BIT);
    CreateAttachment(depthAttachment, VK_FORMAT_D32_SFLOAT,
        VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_IMAGE_ASPECT_DEPTH_BIT);

    return true;
}

bool CreatePhysicallyBasedMaterialDescriptorLayout(
    const VkDevice& vkLogicalDevice, VkDescriptorSetLayout& vkDescriptorSetLayout)
{
    VkDescriptorSetLayoutBinding vkDescriptorSetLayoutBindings[1] = { // Albedo only for now.
    ...
    return vkCreateDescriptorSetLayout(vkLogicalDevice, &vkDescriptorSetLayoutInfo, nullptr,
               &vkDescriptorSetLayout) == VK_SUCCESS;
}

bool SelectVulkanPhysicalDevice(const VkInstance& vkInstance,
    const std::vector<const char*> requiredExtensions, VkPhysicalDevice& vkPhysicalDevice)
{
    ...
    vkEnumerateDeviceExtensionProperties(
        vkPhysicalDevice, nullptr, &supportedDeviceExtensionCount, nullptr);

    std::vector<VkExtensionProperties> supportedDeviceExtensions(supportedDeviceExtensionCount);
    vkEnumerateDeviceExtensionProperties(vkPhysicalDevice, nullptr, &supportedDeviceExtensionCount,
    ...
    for (const auto& requiredExtension : requiredExtensions)
    {
        ...
        spdlog::error(
            "The selected Vulkan physical device does not support required Vulkan Extension: {}",
            requiredExtension);
        return false;
    }
    ...
    VkDevice& vkLogicalDevice)
{
    ...
    for (const auto& requiredExtension : requiredExtensions)
    {
        if (!strcmp(requiredExtension, VK_KHR_TIMELINE_SEMAPHORE_EXTENSION_NAME) &&
            vulkan12Features.timelineSemaphore != VK_TRUE)
            return false;

        if (!strcmp(requiredExtension, VK_KHR_SYNCHRONIZATION_2_EXTENSION_NAME) &&
            vulkan13Features.synchronization2 != VK_TRUE)
            return false;

        if (!strcmp(requiredExtension, VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME) &&
            vulkan13Features.dynamicRendering != VK_TRUE)
            return false;

        if ((strcmp(requiredExtension, VK_EXT_SHADER_OBJECT_EXTENSION_NAME) == 0) &&
    ...
    return vkCreateDevice(vkPhysicalDevice, &vkLogicalDeviceCreateInfo, nullptr,
               &vkLogicalDevice) == VK_SUCCESS;
}

bool LoadByteCode(const char* filePath, std::vector<char>& byteCode)
{
    std::fstream file(
        std::format(R"(..\Shaders\Compiled\{})", filePath), std::ios::in | std::ios::binary);
...
void SetDefaultRenderState(VkCommandBuffer commandBuffer)
{
    ...
    static VkViewport s_DefaultViewport = { 0, kWindowHeight, kWindowWidth, -(int32_t)kWindowHeight,
        0.0, 1.0 };
    ...
    uint32_t& vkQueueIndexGraphics)
{
    ...
    for (uint32_t queueFamilyIndex = 0; queueFamilyIndex < queueFamilyCount; queueFamilyIndex++)
    {
        if (!glfwGetPhysicalDevicePresentationSupport(
                vkInstance, vkPhysicalDevice, queueFamilyIndex))
            continue;
...
void NameVulkanObject(VkDevice vkLogicalDevice, VkObjectType vkObjectType, uint64_t vkObject,
    std::string vkObjectName)
{
    VkDebugUtilsObjectNameInfoEXT attachmentNameInfo = {
